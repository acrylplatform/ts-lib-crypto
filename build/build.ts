...
async function build() {
  try {
    ...
    //await run('tsc', p('tmp'))
    //await copy(p('tmp/dist'), p('tmp/node_modules/acryl-transactions'))
    //await copy(p('package.json'), p('tmp/node_modules/acryl-transactions/package.json'))
    ...
    // const ver = await npmGetVersion('@acryl/ts-lib-crypto')
    // ver.patch++
